	...
	public static void rePosition(MyView view, List<Card> list, int flag) {
		...
		if(flag == 1) {
			for(int i = 0, len = list.size(); i < len; i++) {
				...
				int y = view.screen_height - card.height * 3 / 2;
				// 如果这张牌被选中
				if(card.clicked) {
					// 牌向上移动1/3个牌的高度
					y -= card.height / 3;
		...
		if(flag == 0) {
			for(int i = 0, len = list.size(); i < len; i++){
				Card card = list.get(i);
				card.setLocation(card.width/2 + view.cardHeight,card.height/2+i*card.height/7);
			}
		}
		// 如果是右边玩家牌的集合
		if(flag == 2) {
			for(int i = 0, len = list.size(); i < len; i++){
				Card card=list.get(i);
				card.setLocation(view.screen_width-3*card.width,card.height/2+i*card.height/7);
			}
		}
	}
	/**
